...
CRASH_DISTANCE = 2 # Min distance to obstacle (The LIDAR is in the center of the 1M Rover)
...
class MarsEnv(gym.Env):
    def __init__(self):
        ...
        self.reached_waypoint_7 = False
        self.reached_waypoint_8 = False
    ...
    def reward_function(self):
        ...
        WAYPOINT_1_REWARD = 500
        WAYPOINT_2_REWARD = 1000
        WAYPOINT_3_REWARD = 1500
        WAYPOINT_4_REWARD = 2000
        WAYPOINT_5_REWARD = 2500
        WAYPOINT_6_REWARD = 3000
        WAYPOINT_7_REWARD = 3500
        WAYPOINT_8_REWARD = 4000
        ...
        if self.steps > 0:
            ...
            if self.x < (GUIDERAILS_X_MIN) or self.x > (GUIDERAILS_X_MAX ):
                print("Rover has left the mission map!")
                return 0, True
                
                
            if self.y < (GUIDERAILS_Y_MIN) or self.y > (GUIDERAILS_Y_MAX):
                print("Rover has left the mission map!")
                return 0, True
            
            if self.x > (-5) and self.y < (-4):
                coord=f"({self.x},{self.y})"
                print("Rover has left the mission map!",coord)
            ...
            if self.x > (-13) and self.y < (-7):
                coord=f"({self.x},{self.y})"

                print("Rover has left the mission map!",coord)
                return 0, True  

            if self.x > (-15) and self.y < (-10):
                coord=f"({self.x},{self.y})"

                print("Rover has left the mission map!",coord)
                return 0, True  

            if self.x < (-41) and self.y < (-7):
                coord=f"({self.x},{self.y})"
            ...
            if self.last_position_x <= WAYPOINT_4_X and self.last_position_y <= WAYPOINT_4_Y: # Rover is past the midpoint
                # Determine if Rover already received one time reward for reaching this waypoint
            ...
            if self.last_position_x <= WAYPOINT_5_X and self.last_position_y <= WAYPOINT_5_Y: # Rover is past the midpoint
                # Determine if Rover already received one time reward for reaching this waypoint
            ...
            if self.last_position_x <= WAYPOINT_6_X and self.last_position_y >= WAYPOINT_6_Y: # Rover is past the midpoint
                # Determine if Rover already received one time reward for reaching this waypoint
            ...
            if self.last_position_x <= WAYPOINT_7_X and self.last_position_y >= WAYPOINT_7_Y: # Rover is past the midpoint
                # Determine if Rover already received one time reward for reaching this waypoint
                if not self.reached_waypoint_7:  
                    self.reached_waypoint_7 = True
                    print("Congratulations! The rover has reached waypoint 7!")
                    multiplier = 1 
                    reward = (WAYPOINT_7_REWARD * multiplier)/ self.steps # <-- incentivize to reach way-point in fewest steps
                    return reward, False
                    


            if self.last_position_x <= WAYPOINT_8_X and self.last_position_y >= WAYPOINT_8_Y: # Rover is past the midpoint
                # Determine if Rover already received one time reward for reaching this waypoint
                if not self.reached_waypoint_8:  
                    self.reached_waypoint_8 = True
                    print("Congratulations! The rover has reached waypoint 8!")
                    multiplier = 1 
                    reward = (WAYPOINT_8_REWARD * multiplier)/ self.steps # <-- incentivize to reach way-point in fewest steps
                    return reward, False
            ...
            if self.collision_threshold >= 3.0:      # very safe distance
                multiplier = multiplier + 1
            elif self.collision_threshold < 3.0 and self.collision_threshold >= 2: # pretty safe
                multiplier = multiplier + .5
            elif self.collision_threshold < 2 and self.collision_threshold >= 2.5: # just enough time to turn
    ...
    def callback_wheel_lb(self, data):
        ...
        # print("lin_accel_x= ",lin_accel_x)
        # print("lin_accel_y= ",lin_accel_y)
        # print("lin_accel_z= ",lin_accel_z)
